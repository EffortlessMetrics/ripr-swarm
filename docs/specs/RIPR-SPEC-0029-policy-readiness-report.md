...
| `suppression_health` | Suppressions have identity, owner, reason, scope, dates, and expected visibility; `exposure_gap` entries also have `static_class`. | Suppressions are overbroad, stale, missing owner/reason, missing the exposure-gap class, unknown selectors, or preview suppressions lack preview labels. |
