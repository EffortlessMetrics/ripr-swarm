...
| perl_preview_card_advisory_no_repair_packet | perl | reports/perl-preview-advisory-no-packet.json | perl_preview_card_advisory_only (also `expected_class=reachable_unrevealed`, `must_not_report_clean`, `must_disclose_scope`, no verify/receipt commands, no promotion, and no repair packet) |
