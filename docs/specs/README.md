...
| [RIPR-SPEC-0116](RIPR-SPEC-0116-check-worktree-mode.md) | proposed | `check --worktree` mode: analyze staged and unstaged tracked working-tree edits with `git diff <base>` while keeping committed-history `--base` and file-based `--diff` behavior unchanged; dirty `doctor` guidance recommends `ripr check --base HEAD --worktree`; closes #1296 |
