...
- `status` - `"complete"` when every required artifact is present, no next
  ...
  repair attempt's receipt does not show the gap closed at the current `HEAD`;
  `"incomplete"` when any required artifact is missing or a next command is
  selected. A finished attempt counts toward `"complete"` only when the receipt
  issued for it is `advisory` with static movement `improved` and its evidence
  was recorded at the current `HEAD`.
...
- `repair_attempts[]` - every repair attempt under
  ...
  `repair_receipt_unconfirmed` warning says why).
  - `receipt` - `null` unless the attempt is `ready_to_finish`. Otherwise an
    ...
    packet digests), and, when it does, the receipt's `status`, `movement`,
    `receipt_state`, `recommended_action`, and `analysis_outcome_error`, plus
    `shows_gap_closed` (`advisory` and `improved`). Unbound receipts leave
    these `null` and `shows_gap_closed` `false`.
  - `last_after_refusal` - `null`, or `{reason, recorded_unix_ms}` when the
    ...
    attempt manifest's `last_after_refusal`).
...
- `warnings[]` - stale-looking or unreadable-artifact hints. Timestamp warnings
  ...
  recorded at another `HEAD`, and `repair_receipt_unconfirmed` names a
  `ready_to_finish` attempt whose receipt does not show the gap closed (an
  `invalid` or `incomplete` receipt, a movement that is neither `improved` nor
  open, or no receipt issued for it). Hash mismatch warnings remain a
