...
| `cli/first-pr-front-door-polish` | done | `ripr first-pr` now prints the top repairable gap, changed behavior, why it matters, proof weakness, missing discriminator, focused test intent, verify command, receipt command, artifact paths, static boundary, and concrete recovery/no-action reasons on the first CLI screen. |
