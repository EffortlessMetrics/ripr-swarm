...
| `surface/canonical-state-alignment` | done | User-surface projection receipts now require badge, CLI, LSP, PR comment, review-comment, and CI consumers to preserve canonical runtime status, canonical gap or limitation identity, repair kind, verify command, receipt command, route-quality next actions, advisory posture, and raw-finding non-headline boundaries. This closes the receipt contract, not a blanket renderer rewrite claim. |
| `campaign/lane1-post-08-closeout` | ready | Close the loop with what users can trust, what remains advisory, what remains non-actionable, proof commands, open issues, and explicit non-claims. |
