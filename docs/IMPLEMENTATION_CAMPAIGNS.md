...
| `docs/post-081-support-decision` | ready | Decide whether the post-0.8.1 TypeScript/Bun claim stays preview/advisory or has enough evidence to request a later scoped promotion; the expected current claim remains opt-in preview/advisory calibrated Bun stable-byte evidence, with no TypeScript stable support, Bun UB proof, runtime execution, generated tests, default gates, public repair packets, full Bun binding graph coverage, or support-tier promotion without a separate PR. |
