...
| `first-pr/one-screen-recommendation` | done | Golden-backed first-pr packets now name the top gap/no-action state, changed behavior, current evidence strength, missing discriminator, focused proof intent, verify command, receipt command or unavailable reason, and static-advisory boundary. |
| `outcome/reviewer-native-receipts` | ready | Make `ripr outcome` receipts explain before/after movement and reviewer claims without mutation, runtime, or coverage proof overclaiming. |
