...
| `dogfood/live-bun-stable-byte-receipts` | ready | Add live or live-shaped Bun stable-byte dogfood receipts with tool verdict, manual verdict, route-quality state, placement verdict, proof mode, and operator notes for credited witness, missing discriminator, mention-only rejection, bridge-unknown, and named static limitation cases while preserving preview/advisory authority and no public repair packets. |
