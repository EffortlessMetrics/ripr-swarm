...
| `cli/start-here-command-language` | done | #203 aligned CLI front-door command language, generated workflow summary copy, and `doctor` setup guidance on the converged start-here vocabulary. |
| `receipt/lifecycle-state-convergence` | done | #204 standardizes receipt found/missing/stale/mismatch/improved/unchanged states across the start-here packet, PR evidence ledger, and PR review front panel. |
| `output/no-output-fail-closed-states` | ready | #205 standardizes clean, no-action, missing, stale, wrong-root, disabled, unavailable, malformed, partial, and unsafe states outside the editor. |
