...
| `ops/current-routed-proof-refresh` | done | Current-main routed proof was refreshed on #34 and #24 after PR #372. Recent runs pass through the normalized result with `target=github`; `reason=no_idle_runner` and `reason=runner_image_unavailable` both map to the same bounded blocker: CX53/CX43 are visible but not idle image-ready for `em-ci-rust-1.95`. |
