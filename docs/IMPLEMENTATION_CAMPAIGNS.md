...
| `release/supplemental-denominator` | in progress | #2768 / PR #2790 merged the fail-closed ledger validator and the current-main refresh now records 221 first-parent commits through `224c2408`. PRs #2842 and #2844 are present in development history but retain `safe_defer_post_0_11` / prior `hold_post_release` disposition and `absent_by_candidate_only_exclusion` tree state while #2840/#2841 remain partial. Any corrective commits for those lanes inherit the same disposition unless #2379 explicitly changes the release graph; the ledger remains provisional and does not unblock #1609. |
