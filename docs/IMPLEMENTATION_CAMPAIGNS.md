...
| `docs/bun-ub-first-run-polish` | ready | Update the Bun UB TypeScript preview runbook and docs index for the actual command surface, report artifacts, failure modes, and next actions while keeping known limits explicit and avoiding new flags, analyzer behavior, generated tests, runtime execution, public repair packets, gates, badges, or support-tier promotion. |
