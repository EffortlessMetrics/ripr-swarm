...
| `cli/start-here-command-language` | done | #203 aligned CLI front-door command language, generated workflow summary copy, `doctor` setup guidance, safe-next-action, verification, receipt, recovery-state, and advisory-boundary language without changing packet schema. |
| `receipt/lifecycle-state-convergence` | ready | #204 standardizes receipt found/missing/stale/mismatch/improved/unchanged states. |
