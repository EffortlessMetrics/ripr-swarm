...
| `output/bun-ub-preview-summary` | done | `cargo xtask bun-ub-preview-summary` writes a compact JSON/Markdown Bun UB preview summary from existing graph, calibration, and dogfood data without changing analyzer behavior, public repair-packet authority, generated tests, gates, badges, or support tiers. |
| `agent/bun-cross-language-advisory-packet` | ready | Emit configured Bun cross-language advisory packets that shape agent work or stop on bridge-unknown evidence while keeping `repair_packet_ready = false`. |
