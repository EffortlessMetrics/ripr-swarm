...
| `ops/current-routed-proof-refresh` | done | Current-main routed proof after #352 is recorded on #34 and #24: `Ripr Rust Small Result` passed through GitHub-hosted fallback with `target=github`, `reason=runner_image_unavailable`, and CX53/CX43 visible but not idle image-ready for `em-ci-rust-1.95`. |
