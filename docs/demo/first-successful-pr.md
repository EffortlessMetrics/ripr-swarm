...
-> one repairable stable Rust gap, preview Python/TypeScript gap, or a clear no-action state
-> one repair route
...
| `typescript-preview-gap` | Changed TypeScript behavior is reached by a related Jest context, but the equality boundary is not checked. | Top gap: preview-limited missing boundary assertion for `amount == threshold`. | `AddBoundaryAssertion` in `tests/discount.test.ts`. | `jest tests/discount.test.ts` |
...
## TypeScript Preview Gap Story

The TypeScript preview case proves the same first-run packet can route an
explicit preview `typescript_repair_packet` from saved `ripr check` JSON:
...
Why it matters:
  A related TypeScript test reaches the change, but no boundary discriminator
  was found.
...
The packet stays `preview_limited`, labels the language status as `preview`,
and keeps TypeScript preview evidence advisory-only.
