...
| First Useful PR Loop (28) | Make one changed Rust behavior become one clear repairable gap, one focused proof intent, one verification command, and one reviewer- and agent-readable receipt. | Active: `.ripr/goals/active.toml` selects Campaign 28 with `goals/first-useful-pr-loop-activation` ready and behavior work blocked until the activation PR lands. |

The current machine-readable execution manifest is `.ripr/goals/active.toml`;
it currently records `status = "active"` for Campaign 28, First Useful PR Loop.
The first ready work item is `goals/first-useful-pr-loop-activation`; behavior
work stays blocked until that repo-owned campaign selection lands. The
