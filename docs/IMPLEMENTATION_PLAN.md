...
| Assertion-Target Affinity Owner-Call Tracing (Lane 1 follow-up) | Burn down the measured `call_presence` / `activation_owner_call_absent_assertion_target_affinity` route by adding fixture-backed owner-call tracing while preserving advisory/static-limitation boundaries. | Active: `analysis/assertion-target-affinity-owner-call-tracing` is scoped to unambiguous production one-hop wrappers, including wrappers that call the same owner multiple times, with mixed-owner, ambiguity, local-shadow, and two-hop guards. |
