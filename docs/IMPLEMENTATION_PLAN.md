...
| Value Resolution Static Limits (Lane 1 tracker) | Burn down the scorecard-selected `predicate_boundary` / `activation_value_unresolved` limitation queue from fixture-backed evidence. | Active: [Lane 1 Value Resolution Static Limits](lanes/LANE_1_VALUE_RESOLUTION_STATIC_LIMITS.md), [the implementation plan](../plans/lane1-value-resolution-static-limits/implementation-plan.md), and `.ripr/goals/active.toml` select `fixtures/value-resolution-audit-plan` as the first ready slice before analyzer behavior changes. |
