...
`agent/bun-cross-language-advisory-packet`, and
`output/stable-byte-proof-mode` as completed selected slices. The
next implementation slice is `fixtures/bun-node-fs-scalar-write-profile`. The prior
