...
| Finding Alignment Burn-Down (Lane 1 tracker) | Keep the raw-finding to canonical-item to actionable-gap model useful as new gaps are measured. | Open planning rail: [Lane 1 Finding Alignment Burn-Down](lanes/LANE_1_FINDING_ALIGNMENT_BURNDOWN.md) and [the implementation plan](../plans/lane1-finding-alignment-burndown/implementation-plan.md) define the issue-backed queue. Start with [#1140](https://github.com/EffortlessMetrics/ripr/issues/1140), then burn down measured placement, named-limitation, unsupported-flow, repair-route, verify-command, scorecard, calibration, dogfood, and handoff deltas without reopening already-closed presentation-text or config/policy base scope. |
...
| Start-Here Surface Convergence | Make PR/CI, CLI, editor handoffs, receipts, no-output states, preview promotion criteria, and dogfood receipts lead with the same canonical gap-to-repair unit. | Proposed: [RIPR-PROP-0011](proposals/RIPR-PROP-0011-start-here-surface-convergence.md), [RIPR-SPEC-0053](specs/RIPR-SPEC-0053-start-here-surface-convergence.md), [ADR-0015](adr/0015-start-here-surfaces-use-canonical-gap-records.md), and [the implementation plan](../plans/start-here-surface-convergence/implementation-plan.md) define the burn-down stack. |
...
| Editor Actionable Gap Queue (Lane 3 tracker) | Project existing actionable-gap artifacts into the editor as a bounded local repair queue. | Active: [RIPR-PROP-0013](proposals/RIPR-PROP-0013-editor-actionable-gap-queue.md), [RIPR-SPEC-0055](specs/RIPR-SPEC-0055-editor-actionable-gap-queue.md), [ADR-0017](adr/0017-editor-gap-queue-is-read-only.md), and [the implementation plan](../plans/editor-actionable-gap-queue/implementation-plan.md) define the source-of-truth stack. Validation, Show Status queue projection, Copy Current Repair Packet, Copy Repo Gap Map, fixtures, and VS Code smoke are in place; [Editor actionable gap queue](EDITOR_ACTIONABLE_GAP_QUEUE.md) documents the user workflow before dogfood and closeout. |
