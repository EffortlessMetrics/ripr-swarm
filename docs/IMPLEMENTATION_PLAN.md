...
| Cross-Language Evidence Router UX | Turn the calibrated TypeScript/Bun graph path into a repeatable mixed TypeScript plus Rust operating loop for Bun operators, Claude Code, and other configured projects without promoting preview evidence. | Active: [RIPR-SPEC-0063](specs/RIPR-SPEC-0063-cross-language-evidence-router-ux.md) and [RIPR-PLAN-0063](../plans/cross-language-evidence-router-ux/implementation-plan.md) define PR-sized slices for 0.8.1 patch proof, compact Bun UB summary, advisory agent packet, proof-mode projection, node:fs and Bun.write manifest-only profiles, bridge inventory, live Bun dogfood, runbook polish, and the post-0.8.1 support decision. The patch-proof packet is recorded in [TypeScript/Bun preview patch proof](handoffs/2026-06-05-typescript-bun-preview-patch-proof.md); `.ripr/goals/active.toml` now leaves `output/bun-ub-preview-summary` ready as the next slice. |
...
leaves `output/bun-ub-preview-summary` ready as the next selected successor
slice. The prior post-release successor kept normal development in
