...
| First Useful PR Loop Continuation | Make one changed Rust behavior become one clear repairable gap, one focused proof intent, one verification command, and one reviewer- and agent-readable receipt. | Active: `.ripr/goals/active.toml` selects the First Useful PR Loop continuation; `goals/first-useful-pr-loop-activation` is done and `docs/context-system-proof-stack-reconciliation` is the next ready work item. |
...
`docs/context-system-proof-stack-reconciliation`; behavior work stays blocked
until the repo tracking model is reconciled with the proof-stack terminology.
The
