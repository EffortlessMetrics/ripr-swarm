...
| Python Repair Routing (28) | Turn changed Python behavior into bounded test-repair work with project detection, repair cards, verify commands, agent packets, and outcome receipts while keeping Python preview/advisory until dogfood evidence supports promotion. | Active: [RIPR-PROP-0017](proposals/RIPR-PROP-0017-python-repair-routing-lane.md) and [the Python repair-routing plan](../plans/python-repair-routing/implementation-plan.md) define the staged lane. The current slice makes Python before/after receipt evidence fixture-backed and keeps noise-control fixtures growing: repair cards already project to GapRecords, agent packets, queue items, verify commands, and receipt commands; `ripr swarm ingest` has a Python preview fixture for a test-only closed attempt; `first_successful_pr/python-preview-gap` pins `ripr outcome` JSON/Markdown receipts where the canonical Python gap closes, stays unchanged, opens, strengthens, or weakens across check-output snapshots; `first_successful_pr/python-return-gap` pins non-boundary return-value closure from broad to exact evidence; `first_successful_pr/python-exception-gap` pins exception-path closure from broad exception evidence to exact message evidence; `first_successful_pr/python-field-gap` pins field/object closure from broad object truthiness to exact field evidence; `first_successful_pr/python-output-gap` pins output/log closure from broad output smoke to exact output text evidence; generated Python diffs such as `*_pb2.py` are pinned as excluded from preview repair routing; dynamic import calls such as `importlib.import_module(...)` are pinned as static limitations instead of repair-ready gaps; and metaclass declarations are pinned as metaprogramming static limitations rather than routed repair work. |
