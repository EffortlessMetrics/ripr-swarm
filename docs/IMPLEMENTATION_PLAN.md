...
| Cross-Language Evidence Router UX | Turn the calibrated TypeScript/Bun graph path into a repeatable mixed TypeScript plus Rust operating loop for Bun operators, Claude Code, and other configured projects without promoting preview evidence. | Active: [RIPR-SPEC-0063](specs/RIPR-SPEC-0063-cross-language-evidence-router-ux.md) and [RIPR-PLAN-0063](../plans/cross-language-evidence-router-ux/implementation-plan.md) define PR-sized slices for 0.8.1 patch proof, compact Bun UB summary, advisory agent packet, proof-mode projection, node:fs and Bun.write manifest-only profiles, bridge inventory, live Bun dogfood, runbook polish, and the post-0.8.1 support decision. The 0.8.1 patch proof is recorded in [the TypeScript/Bun preview proof handoff](handoffs/2026-06-05-0.8.1-typescript-bun-preview-patch-proof.md), `cargo xtask bun-ub-preview-summary` now writes the compact advisory summary, configured TypeScript preview cards now include a Bun cross-language advisory packet, configured Bun cross-language grip output now includes advisory stable-byte proof mode, node:fs scalar write is pinned as a manifest-only named limitation, Bun.write is pinned as a helper-gated manifest-only limitation, `cargo xtask configured-bridge-inventory` now lists configured, bridge-unknown, manifest-only, and named-limitation surfaces without analyzer inference, `fixtures/bun-ub-cross-language-dogfood` now records live-shaped stable-byte receipts for configured, missing, mention-only, bridge_unknown, and named-limitation cases, and the Bun UB runbook now gives a copy-pasteable `ripr.toml` plus `ripr doctor/check` first-run loop linked to the summary, inventory, calibration, dogfood, advisory packet, and proof-mode surfaces; next slice is the post-0.8.1 support decision. |

The current machine-readable execution manifest is `.ripr/goals/active.toml`;
it records `status = "active"` for `cross-language-evidence-router-ux` and
...
`dogfood/live-bun-stable-byte-receipts`, and
`docs/bun-ub-first-run-polish` as completed selected slices. The
next implementation slice is `docs/post-081-support-decision`. The prior
