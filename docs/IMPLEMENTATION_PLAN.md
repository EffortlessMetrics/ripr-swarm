...
| First Useful PR Loop Continuation | Make one changed Rust behavior become one clear repairable gap, one focused proof intent, one verification command, and one reviewer- and agent-readable receipt. | Active: `.ripr/goals/active.toml` selects the First Useful PR Loop continuation; `docs/context-system-proof-stack-reconciliation` and `goals/active-goal-freshness-check` are done, and `cli/first-pr-front-door-polish` is the next ready work item. |
...
`cli/first-pr-front-door-polish`; the goal-freshness guardrail is in place, so
behavior work can proceed without following stale closed execution state. The
