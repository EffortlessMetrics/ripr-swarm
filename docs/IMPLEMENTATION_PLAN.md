...
| First Useful PR Loop Continuation | Make one changed Rust behavior become one clear repairable gap, one focused proof intent, one verification command, and one reviewer- and agent-readable receipt. | Active: `.ripr/goals/active.toml` selects the First Useful PR Loop continuation; goal freshness validation is pinned, `cli/first-pr-front-door-polish` landed in #332, `output/one-screen-recommendation-contract` landed in #335, and `receipt/reviewer-native-outcome` is the next ready work item. |
...
`receipt/reviewer-native-outcome`; the goal-freshness guardrail is pinned, the
first-pr front-door stdout behavior landed in #332, and the one-screen
recommendation contract landed in #335. The
