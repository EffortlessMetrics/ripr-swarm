...
| First Useful PR Loop Continuation | Make one changed Rust behavior become one clear repairable gap, one focused proof intent, one verification command, and one reviewer- and agent-readable receipt. | Active: `.ripr/goals/active.toml` selects the First Useful PR Loop continuation; goal freshness validation is pinned, first-pr front-door polish is done, and `output/one-screen-recommendation-contract` is the next ready work item. |
...
`output/one-screen-recommendation-contract`; the goal-freshness guardrail is
pinned, and the first-pr front door now names the selected repair context or
recovery state directly, so the next PR can golden-test the complete
first-screen contract. The
