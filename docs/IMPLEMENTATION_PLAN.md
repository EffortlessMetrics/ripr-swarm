...
| First Useful PR Loop Continuation | Make one changed Rust behavior become one clear repairable gap, one focused proof intent, one verification command, and one reviewer- and agent-readable receipt. | Active: `.ripr/goals/active.toml` selects the First Useful PR Loop continuation; goal freshness validation is pinned, `cli/first-pr-front-door-polish` landed in #332, `output/one-screen-recommendation-contract` landed in #335, `receipt/reviewer-native-outcome` landed in #338, `fixtures/first-pr-demo-story` landed in #341, and `surfaces/first-useful-loop-convergence` is the next ready work item. |

The current machine-readable execution manifest is `.ripr/goals/active.toml`;
it currently records `status = "active"` for the First Useful PR Loop
continuation. The first ready work item is
`surfaces/first-useful-loop-convergence`; the goal-freshness guardrail is pinned, the
first-pr front-door stdout behavior landed in #332, the one-screen
recommendation contract landed in #335, and reviewer-native outcome claim
boundaries landed in #338, and the fixture-backed first successful PR demo
story landed in #341. The
