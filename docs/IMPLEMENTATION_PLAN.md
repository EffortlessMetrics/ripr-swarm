...
| First Useful PR Loop Continuation | Make one changed Rust behavior become one clear repairable gap, one focused proof intent, one verification command, and one reviewer- and agent-readable receipt. | Active: `.ripr/goals/active.toml` selects the First Useful PR Loop continuation; goal freshness validation is pinned and `cli/first-pr-front-door-polish` is the next ready work item. |
...
`cli/first-pr-front-door-polish`; the goal-freshness guardrail is pinned, so
first-pr front-door behavior work may proceed in the next PR. The
