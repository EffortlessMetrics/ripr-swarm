...
| First Useful PR Loop Continuation | Make one changed Rust behavior become one clear repairable gap, one focused proof intent, one verification command, and one reviewer- and agent-readable receipt. | Active: `.ripr/goals/active.toml` selects the First Useful PR Loop continuation; `docs/context-system-proof-stack-reconciliation` is done and `goals/active-goal-freshness-check` is the next ready work item. |
...
`goals/active-goal-freshness-check`; behavior work stays blocked until the
goal-freshness guardrail is in place. The
