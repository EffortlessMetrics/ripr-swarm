...
| Finding Alignment Burn-Down (Lane 1 tracker) | Keep the raw-finding to canonical-item to actionable-gap model useful as new gaps are measured. | Active: [Lane 1 Finding Alignment Burn-Down](lanes/LANE_1_FINDING_ALIGNMENT_BURNDOWN.md), [the implementation plan](../plans/lane1-finding-alignment-burndown/implementation-plan.md), and `.ripr/goals/active.toml` define the current issue-backed queue. Start with [swarm #229](https://github.com/EffortlessMetrics/ripr-swarm/issues/229), then burn down measured placement, named-limitation, unsupported-flow, repair-route, verify-command, scorecard, calibration, dogfood, and handoff deltas without reopening already-closed presentation-text or config/policy base scope. |
