...
oracle quality. User-surface alignment now requires badge, CLI, LSP, PR comment,
review-comment, and CI dogfood receipts to preserve canonical runtime status,
canonical gap or limitation identity, repair kind, verify command, receipt
command, route-quality next actions, advisory posture, and raw-finding
non-headline boundaries. This is the projection receipt contract, not a blanket
claim that every legacy renderer has been rewritten; the active queue resumes at
`campaign/lane1-post-08-closeout` rather than redoing those closed
