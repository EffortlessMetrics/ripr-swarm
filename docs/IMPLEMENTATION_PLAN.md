...
| Assertion-Target Affinity Owner-Call Tracing (Lane 1 follow-up) | Burn down the measured `call_presence` / `activation_owner_call_absent_call_presence_target_affinity` route by adding fixture-backed owner-call tracing while preserving advisory/static-limitation boundaries. | Active: `analysis/call-presence-target-affinity-owner-call-tracing` is scoped to call-presence target-token affinity that remains non-actionable until direct or helper owner-call tracing is supported. The current scoped PR adds the same-package multi-owner production-wrapper sub-shape only when the assertion mentions the matching call target; other-target affinity stays limited. The broader `analysis/assertion-target-affinity-owner-call-tracing` route remains for non-call-presence assertion-target affinity. |
