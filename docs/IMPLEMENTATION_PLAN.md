...
| First Useful PR Loop Continuation | Make one changed Rust behavior become one clear repairable gap, one focused proof intent, one verification command, and one reviewer- and agent-readable receipt. | Active: `.ripr/goals/active.toml` selects the First Useful PR Loop continuation with `goals/first-useful-pr-loop-activation` ready and behavior work blocked until the activation PR lands. |
...
`goals/first-useful-pr-loop-activation`; behavior work stays blocked until that
repo-owned campaign selection lands. The
