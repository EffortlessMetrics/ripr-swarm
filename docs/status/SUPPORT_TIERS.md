...
| Perl repair routing | `scaffold` | None yet — the adapter is fixture-only and not user-facing | [RIPR-PROP-0018](../proposals/RIPR-PROP-0018-perl-repair-routing-lane.md), [RIPR-SPEC-0064](../specs/RIPR-SPEC-0064-perl-fact-packet-contract.md), [ADR 0018](../adr/0018-perl-lsp-fact-substrate.md), [ADR 0019](../adr/0019-language-adapters-reuse-shared-packet-contract.md), Campaign 31 (#1379) | The Perl adapter module is `#[cfg(test)] mod perl;` (`crates/ripr/src/analysis/language/mod.rs:25-26`); `lang-perl` is an empty Cargo feature not in `default`; the production path router recognizes no `.pm`/`.pl`/`.t`/`.psgi` extension; the pipeline returns a fail-closed stub even with the feature on (`crates/ripr/src/analysis/pipeline.rs:289-314`); and the upstream Perl facts exporter does not exist yet. Note: the producer that emits the `ripr-facts` packet is a Perl facts exporter (canonical `perl-ripr-facts`; `perllsp`/`perl-lsp` are compatibility wrappers), not an LSP server. The `perl_preview_card.v1` renderer (`crates/ripr/src/output/perl_preview_card.rs`) is real production code but projects only from synthetic test findings — no production Perl source can feed it. This is intentionally not `preview`: the loop is not useful yet. Promotion path is Campaign 31 (#1379). |
