...
| 5 | [swarm #241](https://github.com/EffortlessMetrics/ripr-swarm/issues/241) / [source #1142](https://github.com/EffortlessMetrics/ripr/issues/1142) | `docs/spec-config-policy-unsupported-flow-expansion` | Refine the criteria for expanding config/policy unsupported-flow support beyond current heuristic supported sinks. | done via swarm #244; selected `opaque_config_lookup` as the next fixture-backed target while preserving the remaining unsupported flows as named limitations |
| 6 | [swarm #246](https://github.com/EffortlessMetrics/ripr-swarm/issues/246) / [source #1143](https://github.com/EffortlessMetrics/ripr/issues/1143) | `fixtures/config-policy-unsupported-flow-burndown` | Add fixture-backed cases for selected config/policy unsupported flows before analyzer expansion. | done via swarm #249; added macro-generated config/schema output and dynamic config dispatch limitation cases while preserving no analyzer behavior change |
