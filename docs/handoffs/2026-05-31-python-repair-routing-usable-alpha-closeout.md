...
| Dogfood receipts | `fixtures/real-repair-attempts` and `fixtures/python-real-repo-evals` record test-only Python repair attempts, focused pytest verify passes, and closed outcome receipts for controlled, normal pytest, CLI/output, API, mixed, and decorated-route cases. |
| Route-quality metrics | `cargo xtask dogfood` measures top-1 usefulness, ranked top-3 precision, verify-command validity, discriminator coverage, suggested-location coverage, false-actionable/crash rates, receipt closure, and unsupported limitation distribution. |
