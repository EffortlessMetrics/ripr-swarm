...
| Dogfood receipts | `fixtures/real-repair-attempts` and `fixtures/python-real-repo-evals` record test-only Python repair attempts, bounded agent packets, focused pytest/unittest verify passes, and closed outcome receipts for controlled, normal pytest, async return-value, parametrized-boundary, CLI/output, log output, argparse CLI output, Click CLI output, Typer CLI output, CLI exit-code, pytest exception, custom exception, and unittest exception paths, API status-code, API JSON detail, Flask route JSON detail, FastAPI route JSON detail, API exception-to-response, mixed, decorated-route, unittest return-value, unittest returned-dict field, and dataclass/model-field cases. The eval corpus also records dynamic-dispatch, decorator-indirection, missing-import-graph, metaprogramming, mocked-module, opaque-custom-helper, property-based, unresolved-fixture, generated-file, and unsupported-syntax no-packet static-limit cases plus no-related-test, already-observed, and heuristic-only ordinary no-action cases so unsupported, excluded, no-path, already-strong, and uncertain-proximity shapes stay visible without becoming repair work. |
| Route-quality metrics | `cargo xtask dogfood` measures top-1 usefulness, ranked top-3 precision, verify-command validity, agent-packet boundary validity, discriminator coverage, suggested-location coverage, false-actionable/crash rates, receipt closure, unsupported limitation distribution, no-action static-limit distribution, and ordinary no-action distribution. |
...
- Post-closeout application slices have now made HTTP/API, CLI/output,
  ...
  repair cards fixture-backed, with multi-field returned dicts now preferring
  literal-valued discriminators such as `status == "active"` over
  pass-through fields such as `name == name`, while preserving fail-closed dynamic/framework
  limits.
- `dogfood/python-stability-evals-v1` is the next checkpoint: extend
  ...
  reasons, false-positive notes, limitation notes, checked no-action
  static-limit cases, and checked ordinary no-action cases.
