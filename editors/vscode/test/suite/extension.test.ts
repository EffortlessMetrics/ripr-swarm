...
suite('Extension Smoke', () => {
  ...
  test('status bar reports server readiness and refresh state', async () => {
    ...
    try {
      ...
      assert.ok(String(context.status.tooltip).includes('Extension state: extension_version_ok (0.8.0)'));
      assert.ok(String(context.status.tooltip).includes('ripr server state: ripr_version_ok (ripr 0.8.0-test)'));
      ...
      assert.ok(String(context.status.tooltip).includes('Server version: ripr 0.8.0-test'));
  ...
  test('status model reports setup files found and missing', async () => {
    ...
    try {
      ...
      assert.ok(statusOutput.includes('ripr server state: ripr_version_ok (ripr 0.8.0-test)'));
      ...
      assert.ok(statusOutput.includes('Server version: ripr 0.8.0-test'));
    } finally {
      await context.dispose();
    }
  });
  ...
  test('editor adoption baseline keeps setup receipt and first-pr projection read-only', async () => {
    await withControllerTestContext({
      ...
      assertReportIncludes(diagnosis, [
        ...
        'Server command: ripr',
        'Server version: ripr 0.8.0-test',
  ...
  test('diagnoseSetup writes read-only setup report', async () => {
    ...
    try {
      ...
      assert.ok(report.includes('ripr server state: ripr_version_ok (ripr 0.8.0-test)'));
      ...
      assert.ok(report.includes('Server version: ripr 0.8.0-test'));
  ...
  test('setup diagnosis reports compatibility states and blocks unsafe repair start', async () => {
    await withControllerTestContext({
      ...
      const report = await diagnoseSetupReport(context);
      assertReportIncludes(report, [
        'Extension state: extension_version_ok (0.8.0)',
        'Expected ripr server version: 0.8.0',
        'ripr server state: ripr_version_too_old (reported ripr 0.5.0-test; expected 0.8.0)',
...
function assertCargoTomlWatcherScopedToWorkspace(
  ...
  assert.strictEqual(watcherPatterns.length, 1, 'expected one repository-config watcher');
