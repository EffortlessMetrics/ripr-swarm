...
normal pytest app repo, one CLI/output-style pytest repo, one exception-path
pytest repo, API-style status-code and JSON-field pytest repos, one mixed
Rust/Python pytest repo, and one decorated route pytest repo before metrics can
support promotion discussion.
