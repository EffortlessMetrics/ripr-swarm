# Golden Output Changes

## Pending — observation_verified_field_construction (1)

Reason:
RIPR-SPEC-0094: FieldConstruction probe with retries token_match stays exposed; initial golden for the anti-over-correction proof fixture

Command:
`cargo xtask goldens bless observation_verified_field_construction --reason "..."`

Updated:
- `expected/check.json`
- `expected/human.txt`

## Pending — observation_verified_field_construction (2)

Reason:
RIPR-SPEC-0109: cap headline confidence by weakest stage's per-stage Confidence (#1219 part D)

Command:
`cargo xtask goldens bless observation_verified_field_construction --reason "..."`

Updated:
- `expected/check.json`
- `expected/human.txt`

## Pending — observation_verified_field_construction (3)

Reason:
bound default human output to start-here triage; human-full preserves exhaustive evidence

Command:
`cargo xtask goldens bless observation_verified_field_construction --reason "..."`

Updated:
- `expected/check.json`
- `expected/human.txt`

## Pending — observation_verified_field_construction (4)

Reason:
#2103: additive changed_files_by_language field and changed_rust_files now Rust-only count

Command:
`cargo xtask goldens bless observation_verified_field_construction --reason "..."`

Updated:
- `expected/check.json`
- `expected/human.txt`

## Pending — observation_verified_field_construction (5)

Reason:
Issue #2598: default human output now exposes bounded explain and context follow-up commands for the selected finding.

Command:
`cargo xtask goldens bless observation_verified_field_construction --reason "..."`

Updated:
- `expected/check.json`
- `expected/human.txt`

## Pending — observation_verified_field_construction (6)

Reason:
Issue #2659: finding navigation commands now preserve the analyzed root, diff or artifact scope and shell-safe identity.

Command:
`cargo xtask goldens bless observation_verified_field_construction --reason "..."`

Updated:
- `expected/check.json`
- `expected/human.txt`

## Pending — observation_verified_field_construction (7)

Reason:
RIPR-SPEC-0076: raise exposed default severity from info to warning so the strongest finding class is not quieter than weaker classes

Command:
`cargo xtask goldens bless observation_verified_field_construction --reason "..."`

Updated:
- `expected/check.json`
- `expected/human.txt`

## Pending — observation_verified_field_construction (8)

Reason:
RIPR-SPEC-0147: publish typed analysis outcome in human and JSON output.

Command:
`cargo xtask goldens bless observation_verified_field_construction --reason "..."`

Updated:
- `expected/check.json`
- `expected/human.txt`

## Pending — observation_verified_field_construction (9)

Reason:
RIPR-SPEC-0147: align fixture outputs with the typed incomplete-outcome and unquoted human outcome contract.

Command:
`cargo xtask goldens bless observation_verified_field_construction --reason "..."`

Updated:
- `expected/check.json`
- `expected/human.txt`

## Pending — observation_verified_field_construction (10)

Reason:
RIPR-SPEC-0125: field-value missing discriminator producer added (#1601)

Command:
`cargo xtask goldens bless observation_verified_field_construction --reason "..."`

Updated:
- `expected/check.json`
- `expected/human.txt`

## Pending — observation_verified_field_construction (11)

Reason:
RIPR-SPEC-0023: classification hint added to digest (#2614)

Command:
`cargo xtask goldens bless observation_verified_field_construction --reason "..."`

Updated:
- `expected/check.json`
- `expected/human.txt`

## Pending — observation_verified_field_construction (12)

Reason:
RIPR-SPEC-0151: additive per-finding source_currentness disposition (#3280); removed-only Rust probes record the base-side line coordinate

Command:
`cargo xtask goldens bless observation_verified_field_construction --reason "..."`

Updated:
- `expected/check.json`
- `expected/human.txt`

## Pending — observation_verified_field_construction (13)

Reason:
RIPR-SPEC-0152: preview findings resolve candidate_current from head-side delta evidence; base-deleted findings gain the human revision label and leave actionable projections

Command:
`cargo xtask goldens bless observation_verified_field_construction --reason "..."`

Updated:
- `expected/check.json`
- `expected/human.txt`

## Pending — observation_verified_field_construction (14)

Reason:
RIPR-SPEC-0152: hidden-count denominator keeps non-current findings; mutation candidates and review-comment authority require candidate_current

Command:
`cargo xtask goldens bless observation_verified_field_construction --reason "..."`

Updated:
- `expected/check.json`
- `expected/human.txt`

## Pending — observation_verified_field_construction (15)

Reason:
RIPR-SPEC-0160: the additive git_candidate_subject identity field (null for ordinary runs) in the check JSON identity block

Command:
`cargo xtask goldens bless observation_verified_field_construction --reason "..."`

Updated:
- `expected/check.json`
- `expected/human.txt`

## Pending — observation_verified_field_construction (16)

Reason:
RIPR-SPEC-0162: the propagation_unknown why-hint stops asserting the propagation the class marks unknown, and unknown-class limitation prose renders under the Analyzer limit label (human-only; the shared decision-layer text is unchanged)

Command:
`cargo xtask goldens bless observation_verified_field_construction --reason "..."`

Updated:
- `expected/check.json`
- `expected/human.txt`

## Pending — observation_verified_field_construction (17)

Reason:
RIPR-SPEC-0001: #3161 PR-B complete direct field witness gate

Command:
`cargo xtask goldens bless observation_verified_field_construction --reason "..."`

Updated:
- `expected/check.json`
- `expected/human.txt`

## Pending — observation_verified_field_construction (18)

Reason:
RIPR-SPEC-0001: normalize field punctuation and exact witness evidence

Command:
`cargo xtask goldens bless observation_verified_field_construction --reason "..."`

Updated:
- `expected/check.json`
- `expected/human.txt`
