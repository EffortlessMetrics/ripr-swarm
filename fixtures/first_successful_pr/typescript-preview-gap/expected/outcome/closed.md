...
- `gap:typescript:typescript_preview:2396aec1` src/discount.ts:2 weakly_gripped -> strongly_gripped (improved; gap closed)
...
- Compared before snapshot fixtures/first_successful_pr/typescript-preview-gap/inputs/reports/before-check.json with after snapshot fixtures/first_successful_pr/typescript-preview-gap/inputs/reports/after-check.json.
- Static seam movement: 1 moved, 0 unchanged, 0 regressed, 0 new, 0 removed.

### What RIPR flagged before?

- weakly_gripped before predicate at src/discount.ts:2.

### What focused proof changed?

- predicate at src/discount.ts:2 shows static evidence movement for focused proof outside RIPR: observe evidence moved from weak to yes; discriminate evidence moved from weak to yes; missing discriminator no longer reported: amount == threshold (equality boundary not observed).

### What moved after verification?

- 1 improved, 0 changed without ranking higher, 0 regressed, 0 unchanged.
- Gap movement: 1 closed, 0 opened, 0 strengthened, 0 weakened, 0 unchanged, 0 new, 0 removed, 0 changed.
- predicate at src/discount.ts:2 moved weakly_gripped -> strongly_gripped (improved).
...
- Open the compared artifacts: fixtures/first_successful_pr/typescript-preview-gap/inputs/reports/before-check.json and fixtures/first_successful_pr/typescript-preview-gap/inputs/reports/after-check.json.
