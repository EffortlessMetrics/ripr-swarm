...
Inputs:
- before: `fixtures/first_successful_pr/python-field-gap/inputs/reports/before-check.json`
- after: `fixtures/first_successful_pr/python-field-gap/inputs/reports/after-check.json`
...
- `gap:python:src/invoice.py:invoice_payload:field_value:field_construction:return_status_paid_id_invoice_id` src/invoice.py:2 weakly_gripped -> strongly_gripped (improved; gap closed)
  ...
  - missing discriminator no longer reported: status == "paid" (changed Python field/object value at line 2 lacks a concrete repair discriminator)
...
- Compared before snapshot fixtures/first_successful_pr/python-field-gap/inputs/reports/before-check.json with after snapshot fixtures/first_successful_pr/python-field-gap/inputs/reports/after-check.json.
- Static seam movement: 1 moved, 0 unchanged, 0 regressed, 0 new, 0 removed.

### What RIPR flagged before?

- weakly_gripped before field_value at src/invoice.py:2.

### What focused proof changed?

- field_value at src/invoice.py:2 shows static evidence movement for focused proof outside RIPR: observe evidence moved from weak to yes; discriminate evidence moved from weak to yes; missing discriminator no longer reported: status == "paid" (changed Python field/object value at line 2 lacks a concrete repair discriminator).

### What moved after verification?

- 1 improved, 0 changed without ranking higher, 0 regressed, 0 unchanged.
- Gap movement: 1 closed, 0 opened, 0 strengthened, 0 weakened, 0 unchanged, 0 new, 0 removed, 0 changed.
- field_value at src/invoice.py:2 moved weakly_gripped -> strongly_gripped (improved).
...
- Open the compared artifacts: fixtures/first_successful_pr/python-field-gap/inputs/reports/before-check.json and fixtures/first_successful_pr/python-field-gap/inputs/reports/after-check.json.
...
- RIPR compared only the listed static snapshots: fixtures/first_successful_pr/python-field-gap/inputs/reports/before-check.json and fixtures/first_successful_pr/python-field-gap/inputs/reports/after-check.json.
