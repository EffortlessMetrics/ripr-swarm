...
- Activation-value resolution must not invent synthetic values. Direct owner
  ...
  that directly call the owner. Predicate-boundary checks still require
  concrete activation values, and builder or fixture override methods count
  only when their method tokens align with the changed seam parameters.
