...
| RIPR swarm plan packet corpus | `fixtures/swarm-plan-packet-corpus` | The swarm planner ranks canonical actionable packets and blocks unsafe or incomplete packets before repair execution. | Manifest-only packet corpus: `SPEC.md` and `corpus.json` pin ready boundary/error/output packets, static-limitation blocking, missing verify/receipt routes, and missing must-not-change boundaries. |
| Actionable gap outcomes corpus | `fixtures/actionable-gap-outcomes-corpus` | The outcome report joins actionable packets to receipts and evidence movement without executing repairs. | Manifest-only outcome corpus: `SPEC.md` and `corpus.json` pin not-attempted, receipt-present, improved, unchanged, regressed, resolved, and attempted-without-receipt states. |
