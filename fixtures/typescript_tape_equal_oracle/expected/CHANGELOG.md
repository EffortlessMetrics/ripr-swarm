...
Reason:
pin TypeScript execution-context tape equal exact-value oracle
...
## Pending — typescript_tape_equal_oracle (2)
...
## Pending — typescript_tape_equal_oracle (3)
...
Reason:
RIPR-SPEC-0098 value-sink extension: ReturnValue exposure promotion now requires the
confirming strong assertion to observe the changed sink (owner reference or changed
token in observed_expression). These fixtures' tests assert a bare local
(`expect(result)` / `t.is(result, ...)`) with no owner reference and no changed token,
so the honest static class is weakly_exposed with a propagation_unknown named
limitation and a return-value missing discriminator, not exposed. The flip is the guard
working as intended.
