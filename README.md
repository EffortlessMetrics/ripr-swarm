...
| RIPR swarm repair loop | `cargo xtask ripr-swarm plan --top <n>` ranks actionable canonical gap packets into swarm-ready, blocked, missing-context, static-limitation, and high-confidence buckets without editing files, running tests, calling providers, generating tests, creating receipts, running mutation testing, changing PR/CI rendering, changing LSP/editor behavior, changing gates, or changing public badges. The swarm plan packet and actionable-gap outcomes corpora pin ready packets, blocked packets, and not-attempted / receipt-present / improved / unchanged / regressed / resolved outcome joins. | Receipt-to-gap attempt history and orphan receipt reporting. |
