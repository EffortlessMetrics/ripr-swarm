...
| Python preview | Opt-in Python preview facts cover project/test discovery, owners, probes, oracle shapes, static limits, related-test hints, and stable canonical gap IDs for non-static-limit findings across the current output surfaces. Static-limit findings remain advisory limitation evidence, not repair cards. | `analysis/python-ripr-evidence-model` |
