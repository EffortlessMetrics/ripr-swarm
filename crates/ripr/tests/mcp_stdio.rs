...
fn run_mcp(root: &Path, chunks: &[&[u8]]) -> Result<Output, String> {
    ...
    for chunk in chunks {
        stdin
            .write_all(chunk)
            .map_err(|error| format!("write MCP request chunk: {error}"))?;
        stdin
            .flush()
            .map_err(|error| format!("flush MCP request chunk: {error}"))?;
    }
    drop(stdin);
    ...
    loop {
            ...
            .map_err(|error| format!("poll ripr mcp: {error}"))?
        {
            Some(_status) => {
                return child
                    .wait_with_output()
                    .map_err(|error| format!("collect ripr mcp output: {error}"));
            }
            ...
            None => {
                ...
                let output = child
                    .wait_with_output()
                    .map_err(|error| format!("collect timed-out ripr mcp output: {error}"))?;
                return Err(format!(
...
fn current_discovery_requires_metadata_and_rejects_legacy_ping() -> Result<(), String> {
    ...
    if responses[3].pointer("/error/code").and_then(Value::as_i64) != Some(-32601) {
