...
fn write_bound_repo_exposure_fixture(
  ...
  "artifact": {{
    ...
    "analysis": {{"format": "repo-exposure-json", "mode": "draft", "base_revision": null, "input_identity": "input:v2:fixture", "command": "ripr check --format repo-exposure-json", "profile": "draft", "worktree": "clean"}},
    "snapshot_identity": "snapshot:input:v2:fixture;revision:{head}",
...
fn bind_repo_exposure_fixture_with_worktree(
    ...
    value["artifact"] = serde_json::json!({
        ...
        "producer": {"tool": "ripr", "version": "0.10.0"},
        "repository": {"root": root_identity, "head": head},
        "analysis": {"format": "repo-exposure-json", "mode": "draft", "base_revision": null, "input_identity": "input:v2:fixture", "command": "ripr check --format repo-exposure-json", "profile": "draft", "worktree": worktree},
        "snapshot_identity": format!("snapshot:input:v2:fixture;revision:{head}"),
        "content_sha256": placeholder,
    });
...
fn agent_verify_rejects_incomparable_analysis_inputs() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let altered = std::fs::read_to_string(&after)?
        ...
        .replace(
            &format!("snapshot:input:v2:fixture;revision:{old_head}"),
            &format!("snapshot:input:v2:other;revision:{new_head}"),
        )
        .replace(
            "\"input_identity\": \"input:v2:fixture\"",
            "\"input_identity\": \"input:v2:other\"",
        );
