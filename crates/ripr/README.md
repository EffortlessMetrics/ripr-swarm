...
| --- | --- | --- |
| Distribution | `0.11.0` is the current release (published on crates.io via `cargo install ripr` at publication). The source-of-truth release authority is `EffortlessMetrics/ripr`; `ripr-swarm` is the development trunk. GitHub Releases in this repo are development artifacts, not the distribution channel. Rust 1.95 MSRV. | Promotion and release-maintenance proof. |
