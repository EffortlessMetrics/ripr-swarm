...
pub(crate) fn render_pilot_timeout_summary_json(context: PilotSummaryContext<'_>) -> String {
    ...
    push_json_config(&mut out, context);
    push_json_outputs(&mut out, context);
...
fn push_json_config(out: &mut String, context: PilotSummaryContext<'_>) {
    out.push_str("  \"config\": {");
    match context.config_path {
...
fn push_json_outputs(out: &mut String, context: PilotSummaryContext<'_>) {
    out.push_str("  \"outputs\": {\n");
    push_path_field(
        out,
        "repo_exposure_json",
