...
use crate::cli::commands::write::write_text_file;
...
pub(super) fn agent(args: &[String]) -> Result<(), String> {
    ...
    if let Some(result) = dispatch::run_agent_help_command(&command) {
        return result;
    }
    ...
    match command {
        ...
        | AgentCommand::ReviewSummaryHelp) => dispatch::run_agent_help_command(&help_command)
