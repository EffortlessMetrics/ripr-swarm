...
//!
//! See #1771 / #1614. The report captures the top-level checks (root,
//! Cargo.toml, tool availability) as typed `DoctorCheck` values and
//! wraps the deeper sub-checks (languages, cache, config, perl, test
//! surfaces) as text-based `DoctorSection` entries. A follow-up PR can
//! type each section individually; the structure here proves the dual
