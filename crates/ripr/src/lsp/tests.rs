...
#[test]
fn post_adoption_editor_contract_pins_rust_default_projection() -> Result<(), String> {
    let fixture_root = boundary_gap_fixture_root();
    let config = boundary_gap_lsp_config(crate::config::RiprConfig::default());
    let projection = workspace_projection_contract(&fixture_root, &config)?;

    let diagnostics = projection["diagnostics"]
        .as_array()
        .ok_or_else(|| "expected diagnostics array".to_string())?;
    let diagnostic = diagnostics
        .iter()
        .find(|diagnostic| diagnostic["code"] == "ripr-seam-weakly-gripped")
        .ok_or_else(|| {
            format!(
                "missing Rust seam diagnostic:\n{}",
                pretty_json(&projection)
            )
        })?;
    assert_eq!(diagnostic["uri"], "src/lib.rs");
    assert_eq!(diagnostic["source"], "ripr");
    assert_eq!(diagnostic["severity"], "warning");
    assert_eq!(diagnostic["data"]["seam_id"], "67fc764ba37d77bd");
    assert_eq!(diagnostic["data"]["grip_class"], "weakly_gripped");
    assert_eq!(diagnostic["data"]["headline_eligible"], true);

    let hover = projection["hover"]
        .as_str()
        .ok_or_else(|| "expected hover markdown".to_string())?;
    assert!(
        hover.contains("**ripr** behavioral seam")
            && hover.contains("`weakly_gripped`")
            && hover.contains("## Missing discriminator")
            && hover.contains("Analysis snapshot: generated <elapsed> ago."),
        "unexpected Rust default hover:\n{hover}"
    );

    let actions = projection["actions"]
        .as_array()
        .ok_or_else(|| "expected action array".to_string())?;
    ...
    assert_eq!(
        ...
        "Rust default editor action ladder drifted"
    );

    let status = projection["status"]
        .as_str()
        .ok_or_else(|| "expected refresh status".to_string())?;
    assert!(
        status.contains("seam_diagnostics=1")
            && status.contains("published_files=1")
            && status.contains("enabled_language_names=rust"),
        "unexpected Rust default status projection: {status}"
    );
    Ok(())
}
