...
fn limitation_of_kind(
    ...
    parsed
        ...
        .unwrap_or_else(|| panic!("missing limitation {kind:?}: {:#?}", parsed.limitations))
}

fn changed_file<'a>(parsed: &'a ParsedDiff, path: &str) -> &'a super::ChangedFile {
    parsed
        .changed_files
        .iter()
        .find(|file| file.path == Path::new(path))
        .unwrap_or_else(|| panic!("missing changed file {path}: {:#?}", parsed.changed_files))
}

#[test]
fn combined_cc_hunk_is_quarantined_and_typed() {
...
#[test]
fn explicit_combined_hunk_uses_the_same_typed_limitation() {
    let parsed = parse_unified_diff_with_metadata(
...
#[test]
fn combined_hunk_does_not_consume_the_following_ordinary_file() {
    let parsed = parse_unified_diff_with_metadata(
...
#[test]
fn ordinary_file_before_combined_hunk_keeps_path_attribution_separate() {
    let parsed = parse_unified_diff_with_metadata(
        "diff --git a/src/lib.rs b/src/lib.rs\n\
         --- a/src/lib.rs\n\
         +++ b/src/lib.rs\n\
...
#[test]
fn added_conflict_region_is_quarantined_and_typed() {
    let parsed = parse_unified_diff_with_metadata(
        "diff --git a/src/lib.rs b/src/lib.rs\n\
         --- a/src/lib.rs\n\
         +++ b/src/lib.rs\n\
...
#[test]
fn removed_conflict_region_is_quarantined_and_typed() {
    let parsed = parse_unified_diff_with_metadata(
        "diff --git a/src/lib.rs b/src/lib.rs\n\
         --- a/src/lib.rs\n\
         +++ b/src/lib.rs\n\
...
#[test]
fn unclosed_conflict_region_remains_limited_at_end_of_input() {
    let parsed = parse_unified_diff_with_metadata(
        "diff --git a/src/lib.rs b/src/lib.rs\n\
         --- a/src/lib.rs\n\
         +++ b/src/lib.rs\n\
...
#[test]
fn isolated_separator_and_marker_text_inside_source_do_not_false_trigger() {
    let parsed = parse_unified_diff_with_metadata(
        "diff --git a/src/lib.rs b/src/lib.rs\n\
         --- a/src/lib.rs\n\
         +++ b/src/lib.rs\n\
...
#[test]
fn ordinary_two_way_hunk_output_and_existing_counters_remain_unchanged() {
    let parsed = parse_unified_diff_with_metadata(
        "diff --git a/src/lib.rs b/src/lib.rs\n\
         --- a/src/lib.rs\n\
         +++ b/src/lib.rs\n\
