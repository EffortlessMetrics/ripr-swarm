...
use crate::domain::{
    ...
    MissingDiscriminatorFact, OracleKind, OracleStrength, OwnerKind, Probe, ProbeFamily,
    RevealEvidence, RiprEvidence, SourceLocation, StageEvidence, StageState, StaticLimitKind,
    StopReason, SymbolId,
};
#[cfg(test)]
use crate::domain::{DeltaKind, FlowSinkKind, RelatedTest};
...
use discriminators::{
    ...
    python_exit_code_discriminator, python_missing_discriminators,
    python_return_constructor_field_discriminator, python_return_dict_field_discriminator,
    python_string_literal_value, split_python_assignment, top_level_python_segments,
};
...
#[cfg(test)]
use probe_shape::{contains_mock_initializer, looks_like_call_expression};
use related_tests::{
    PythonRelatedCandidate, find_related_tests, first_parenthesized_string_argument,
    import_source_module_matches_owner, line_prefix_looks_like_comment_or_string,
    python_repair_placement, related_test_candidates,
    strong_test_calls_owner_method_on_bound_receiver, strong_test_imports_owner_from_module,
    strongest_assertion, verify_command_for_test,
};
#[cfg(test)]
use related_tests::{
    PythonRelationKind, binding_target_for_construction, body_calls_owner,
    construct_result_is_called, contains_any_attribute_call, has_unclosed_quote,
    imported_module_matches_owner, local_binding_calls_owner, normalize_similarity_key,
    normalize_test_stem, owner_similarity_keys, related_test_relation, same_stem_related,
    similarity_key_contains,
};
use sink_alignment::{SinkAlignment, classify_sink_alignment_with_old};
#[cfg(test)]
use sink_alignment::{
...
use source_facts::{extract_source_facts, parse_module_result, source_fact_snapshot_observation};
mod source_utils;
#[cfg(test)]
...
mod static_limits;
use static_limits::{
    PythonStaticLimit, has_identifier_boundary, is_simple_python_identifier, line_prefix_before,
    python_callee_start_has_boundary, python_prefix_hides_code, static_limit_for_change,
};
#[cfg(test)]
use static_limits::{
    ...
    is_transparent_owner_decorator_for_owner, line_uses_imported_symbol, test_has_mocked_module,
};
