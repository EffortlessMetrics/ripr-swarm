...
10. Do not update a behind-only branch. Reconcile only an actual conflict, explicit stack change, material prerequisite change, or failed combined-tree proof. Before resolving a reported conflict, `git log origin/main` the seam for an already-landed equivalent; on a verified-broader landed equivalent, close as duplicate instead of resolving.
