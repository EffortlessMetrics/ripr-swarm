...
- Commands run under a POSIX shell with GNU coreutils. Know which shell:
  `${PIPESTATUS[0]}` below needs Bash; plain POSIX `sh` (dash) lacks it.
- A pipeline hides the producer's exit code: `cargo test … | grep …`
  ...
  success. Under Bash, inspect the producer's result lines or capture
  `${PIPESTATUS[0]}` before the pipe. Under plain `sh`, restructure to
  avoid the pipe instead.
