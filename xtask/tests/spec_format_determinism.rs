...
fn finish(root: PathBuf, output: std::process::Output, expected_success: bool) -> Result<(), String> {
...
fn structural_spec_errors_remain_blocking() -> Result<(), String> {
    ...
    if output.status.success() {
        return Err(format!("missing required heading passed unexpectedly: {details}"));
    }
    if !details.contains("missing `## Metrics`") {
        return Err(format!("missing-heading diagnostic was not preserved: {details}"));
    }
    Ok(())
}
