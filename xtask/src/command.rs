...
pub(crate) fn known_commands() -> Vec<&'static str> {
    vec![
        ...
        "bun-ub-preview-summary [--graph-corpus <path>] [--calibration-corpus <path>] [--dogfood-corpus <path>] [--out <path>] [--out-md <path>]",
...
pub(crate) fn command_catalog() -> Vec<CommandCatalogEntry> {
    vec![
        ...
        command_entry(
            ...
            false,
            "Writes the compact advisory Bun UB preview summary from existing graph, calibration, and dogfood data; does not run Bun, TypeScript, mutation, providers, generated tests, source edits, gates, badges, or support-tier promotion.",
        ),
        command_entry(
            "recommendation-calibration [--root <path>] [--pr-guidance <path>] [--outcome-receipts <path>] [--out <path>]",
            "report_only",
