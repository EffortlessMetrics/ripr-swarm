...
impl XtaskCommand {
    pub(crate) fn parse(args: impl IntoIterator<Item = String>) -> Self {
        ...
        match command.as_str() {
            ...
            "active-goal-authority-audit" => Self::ActiveGoalAuthorityAudit,
...
pub(crate) fn command_catalog() -> Vec<CommandCatalogEntry> {
    vec![
        ...
        command_entry(
            "active-goal-authority-audit",
            "report_only",
            "target/ripr/reports/active-goal-authority-audit.{md,json}",
            false,
            "Inventories singleton active-goal consumers and reports migration blockers without changing goal authority or repository state.",
        ),
        command_entry(
            "pr-body --work-item <id>",
            "report_only",
