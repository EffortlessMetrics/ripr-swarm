...
pub(crate) fn known_commands() -> Vec<&'static str> {
    vec![
        ...
        "windows-advisory-summary --run1 <path> --run2 <path>",
        "suggested-fixes",
        "precommit",
        "check-pr",
        "fixtures [name]",
...
pub(crate) fn command_catalog() -> Vec<CommandCatalogEntry> {
    vec![
        ...
        command_entry(
            ...
            false,
            false,
            "Turns two `cargo test --workspace` logs from the advisory Windows lane into a Markdown verdict on stdout: separates deterministic failures (failed in both runs) from unstable ones (failed in exactly one), names a build failure as infrastructure rather than a product verdict, reports which test targets actually ran because cargo stops after the first failing target, and reports an absent log as unknown rather than as a pass. Reads nothing but the two logs and writes no files.",
        ),
        command_entry(
            "suggested-fixes",
            "report_only",
