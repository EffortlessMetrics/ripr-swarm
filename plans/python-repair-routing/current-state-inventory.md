...
| `cli_output_optional` | `python_pytest_oracle_shapes`, `python_call_argument_shape`, `python_cli_output_repair_gap`, `python_argparse_output_repair_gap`, `first_successful_pr/python-output-gap`, `python-real-repo-evals:cli_exit_code_pytest_receipt` | Generic call, log/output, and side-effect shapes can carry missing discriminators such as `log contains "coupon expired"` or `call includes "receipt.sent"` when direct weak evidence exists, with generic placement/verify guidance; simple Click CLI output keeps known static decorators transparent, routes `click.echo(...)` to `output contains ...`, and pins pytest placement/verify guidance; argparse-shaped command setup can route a changed static `print(...)` output to the same bounded pytest CLI output repair card without executing argparse; exact output text equality can close the output/log canonical gap while containment remains conservative weak evidence; literal `sys.exit(...)` / `SystemExit(...)` discriminators get CLI exit-code repair-card copy, and the dogfood corpus now records a focused pytest exit-code repair whose bounded packet edits only `tests/test_commands.py`, forbids `src/commands.py`, verifies the suggested test node, and closes the canonical call/output-effect gap. | Add richer framework-runner repair packets only when command construction is statically bounded. |
