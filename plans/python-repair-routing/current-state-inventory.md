...
| Outcome/ledger | `ripr outcome` can compare check-output findings by Python canonical gap ID; `first_successful_pr/python-preview-gap` pins closed, unchanged, opened, strengthened, and weakened predicate-boundary receipts, and `first_successful_pr/python-return-gap` pins return-value gap closure in JSON and Markdown. Strengthened rows that still need attention remain visible in the receipt's weak/unknown section. | Add exception, field/object, output/log, and real dogfood receipts before support-tier promotion. |
