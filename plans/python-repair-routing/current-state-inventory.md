...
return-gap fixture proves a non-boundary return-value gap can close when broad
evidence becomes an exact return assertion.
...
| Outcome/ledger | `ripr outcome` can compare check-output findings by Python canonical gap ID; `first_successful_pr/python-preview-gap` pins closed, unchanged, opened, strengthened, and weakened predicate-boundary receipts, `first_successful_pr/python-return-gap` pins return-value gap closure, and `first_successful_pr/python-exception-gap` pins exception-path closure when a broad exception observer becomes an exact message observer. Strengthened rows that still need attention remain visible in the receipt's weak/unknown section. | Add field/object, output/log, and real dogfood receipts before support-tier promotion. |
