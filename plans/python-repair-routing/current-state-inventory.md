...
`docs/python-current-state-inventory`. It started as the Python preview
baseline and is kept current as the repair-routing lane lands scoped slices.
...
weak repair cards, bounded agent packets, verify-command metadata, first-use
projection, before/after receipts, report-only no-action projection, and dogfood
quality metrics. The remaining work is concentrated in additional stability
dogfood, broader application-shaped receipts, and keeping Python-specific
surfaces aligned without promoting broader Python static facts beyond preview.
...
| SARIF output | [`crates/ripr/src/output/sarif.rs`](../../crates/ripr/src/output/sarif.rs) | Renders diff-scoped findings by file/line and RIPR fields, carries Python language/status/static-limit metadata, projects eligible `python_repair_card` properties, and marks Python `already_observed`, `no_related_test`, `heuristic_only`, and static-limit findings with explicit `python_no_action` no-packet metadata. |
| GitHub annotation output | [`crates/ripr/src/output/github.rs`](../../crates/ripr/src/output/github.rs) | Emits concise workflow-command annotations for eligible Python repair cards and for Python preview no-action states, explicitly naming no repair card or agent packet for already-observed, no-related-test, heuristic-only, and static-limit findings. |
...
| --- | --- | --- |
| Source snapshots | Stable file/span/language facts for modules, classes, functions, methods, decorators, parameters, returns, raises, predicates, comparisons, boolean expressions, calls, assignments, attribute writes, dict/list/set literals, string literals, and print/log calls. | Snapshot facts remain internal analysis substrate; user-facing canonical gaps and repair cards are projected from the downstream probe, evidence, placement, and oracle layers. |
| Owners | Top-level `def`, `async def`, methods, `@staticmethod`, `@classmethod` methods, class-body owners, module-level owners, and stable `python:<path>::<qualified_owner>` probe owner IDs. | Class owners intentionally omit `owner_kind` until the shared vocabulary adds a class value; repair-gap IDs use stable language-qualified owner paths where available. |
...
| Pytest oracles | `assert a == b`, boundary comparisons, field assertions, output observers through `caplog` / `capsys`, status-code and exit-code assertions, bare `assert expr`, custom `assert_*` helpers, `isinstance(...)`, broad `pytest.raises(...)` / imported `raises(...)`, exact `pytest.raises(..., match=...)`, and `pytest.mark.parametrize` presence. | Boundary discriminator extraction is limited to simple syntax-derived predicate comparisons; simple response JSON observer repair cards are covered for statically linked route/client shapes, while dynamic response helpers remain planned. |
| Unittest oracles | `assertEqual`, `assertNotEqual`, `assertTrue`, `assertFalse`, broad `assertRaises`, exact `assertRaisesRegex`, `assertIn`, `assertRegex`, `assertDictEqual`, and unittest verify-command evidence. | Placement and command confidence are available for direct weak unittest findings; richer command construction remains limited to syntactic selectors. |
...
| Probe shapes | Predicate/control, return value, error path, field assignment, returned dict fields, side-effect calls, await calls, and mock initializer shapes. | Canonical Python gap IDs now identify non-static-limit preview findings by language, file, owner, behavior kind, probe kind, and normalized discriminator; repair cards and before/after receipts are fixture-backed for the first repair classes, with future packs expected to add receipts before broader claims. |
...
- owner and routing fixtures:
  ...
  [`fixtures/python_no_config_repair_gap`](../../fixtures/python_no_config_repair_gap),
...
| `pytest_smoke_assert` | `python_boundary_gap`, `python_broad_boolean_assertion`, `python-real-repo-evals:*_receipt` rows with `repair_action = "strengthen_existing_test"` | Unknown, reach-only, or smoke oracle keeps finding `weakly_exposed`, JSON evidence records the non-exact oracle shape, and direct weak findings with a safe target now prefer `strengthen_existing_test` repair cards instead of duplicate-test guidance. | Keep strengthening recommendations bounded to cases with a concrete discriminator, related test location, verify command, stop conditions, and no production-code edit requirement. |
...
| SARIF | Renders diff finding locations and RIPR properties, preserves Python language/status/static-limit metadata, carries eligible `python_repair_card` context, and marks Python ordinary no-action states plus static-limit findings as `python_no_action` without verify, receipt, or packet authority. | Keep future Python repair packs and typed no-action states aligned with SARIF without making code scanning a repair executor or gate. |
| GitHub annotations | Render concise advisory workflow-command text for eligible Python repair cards and for Python preview ordinary no-action/static-limit states, including the no-repair-card/no-agent-packet boundary. | Keep the annotation message as review context only; do not turn annotations into packet, verify, receipt, or gate authority. |
...
| PR summary/front panel | Can consume existing report/gap artifacts, highlights the top Python preview repair card from `actionable-gaps.json` with the canonical gap, changed owner, missing discriminator, suggested test target, verify command, receipt command, stop conditions, and advisory boundary, and surfaces top report-only Python no-action/static-limit gap-ledger states as `no_repair_packet` guidance instead of generic silence. | Keep newly added repair packs and no-action states wired through the same start-here/front-panel surfaces. |
...
| Dogfood quality metrics | `cargo xtask dogfood` reads `fixtures/python-real-repo-evals/corpus.json` and derives Python repair-routing quality metrics for top-1 actionable usefulness, top-3 actionable precision over captured ranked repair-card findings, verify-command validity, agent-packet boundary validity, concrete discriminator coverage, suggested test-location coverage, false-actionable rate, crash rate, receipt closure rate, unsupported limitation distribution, no-action static-limit distribution, and ordinary no-action distribution. Cases with fewer than three ranked repair cards must record a limit reason, at least one checked eval must preserve a full top-3 repair-card capture, `static_limit_cases` must prove no card, no packet, no verify claim, and no receipt movement for no-action evals, and `no_action_cases` must prove the same no-packet/no-receipt boundary for ordinary stops such as no related static test path, already-observed exact evidence, and heuristic-only related-test proximity. The required receipt set now includes boundary, no-config pyproject project detection, external-repo-style `src/` package layout, multi-card top-3 capture, async return-value, CLI/output, log output, argparse CLI output, Click CLI output, Typer CLI output, CLI exit-code, pytest and unittest exception paths, API status, API JSON detail, Flask route JSON detail, FastAPI route JSON detail, API exception-response, mixed Rust/Python, decorated route, unittest return-value, unittest dict-field, and model-field closure rows. | Use the ranked precision, packet-boundary, receipt, fail-closed static-limit, and ordinary no-action corpus during future stability decisions. |
...
no-action states such as no related static test path and already-observed exact
evidence should stay in `no_action_cases`. Neither bucket should inflate
repair-card success metrics or route broader preview facts into gates, badges,
baselines, or RIPR Zero.
