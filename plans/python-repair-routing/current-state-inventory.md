...
strengthened/weakened receipt movement for the predicate-boundary gap.
...
| Outcome/ledger | `ripr outcome` can compare check-output findings by Python canonical gap ID, and `first_successful_pr/python-preview-gap` pins closed, unchanged, opened, strengthened, and weakened Python preview gap receipts in JSON and Markdown. Strengthened rows that still need attention remain visible in the receipt's weak/unknown section. | Add non-boundary receipt fixtures and real dogfood receipts before support-tier promotion. |
