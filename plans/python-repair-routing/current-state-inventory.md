...
| SARIF output | [`crates/ripr/src/output/sarif.rs`](../../crates/ripr/src/output/sarif.rs) | Renders diff-scoped findings by file/line and RIPR fields, carries Python language/status/static-limit metadata, projects eligible `python_repair_card` properties, and now marks Python static-limit findings with explicit `python_no_action` no-packet metadata. |
...
| SARIF | Renders diff finding locations and RIPR properties, preserves Python language/status/static-limit metadata, carries eligible `python_repair_card` context, and marks Python static-limit findings as `python_no_action` analyzer limitations without verify, receipt, or packet authority. | Keep future Python repair packs and typed no-action states aligned with SARIF without making code scanning a repair executor or gate. |
