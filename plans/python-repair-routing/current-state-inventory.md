...
| LSP/editor | Preview routing and metadata projection exist for Python. Python preview GapRecord diagnostics with safe `pytest ...` or `python -m unittest ...` verify commands can now expose verify/receipt actions; pytest-backed records also expose a copyable pytest skeleton and open the target test file even when the repair route carries a bare test name. | Add copyable full repair-card text and stale-state warnings for Python-specific cards. |
