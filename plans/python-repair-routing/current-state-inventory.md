...
| `predicate_boundary` | `python_boundary_gap`, `python_strong_oracle`, `python_parametrize_basic`, `python_test_placement_verify`, `first_successful_pr/python-preview-gap` | Predicate probe, weak/strong related-test examples, canonical gap identity, simple equality-boundary missing discriminators such as `amount == threshold`, pytest placement/verify guidance, repair-card projection, and a before/after `ripr outcome` fixture where the canonical Python gap closes when the equality discriminator is no longer missing. | Add broader unchanged/regressed/opened Python receipt fixtures before dogfood promotion. |
...
| Outcome/ledger | `ripr outcome` can compare check-output findings by Python canonical gap ID, and `first_successful_pr/python-preview-gap` pins a closed Python preview gap receipt in JSON and Markdown. | Expand Python receipt fixtures for opened, unchanged, strengthened, weakened, and regressed movement before support-tier promotion. |
