...
| Dogfood quality metrics | `cargo xtask dogfood` reads `fixtures/python-real-repo-evals/corpus.json` and derives Python repair-routing quality metrics for top-1 actionable usefulness, top-3 actionable precision over captured ranked repair-card findings, verify-command validity, agent-packet boundary validity, concrete discriminator coverage, suggested test-location coverage, false-actionable rate, crash rate, receipt closure rate, unsupported limitation distribution, and no-action static-limit distribution. Cases with fewer than three ranked repair cards must record a limit reason, and `static_limit_cases` must prove no card, no packet, no verify claim, and no receipt movement for no-action evals. The required receipt set now includes boundary, CLI/output, Click CLI output, Typer CLI output, CLI exit-code, pytest and unittest exception paths, API status, API JSON detail, Flask route JSON detail, API exception-response, mixed Rust/Python, decorated route, unittest return-value, and model-field closure rows. | Use the ranked precision, packet-boundary, receipt, and fail-closed static-limit corpus during future stability decisions. |
