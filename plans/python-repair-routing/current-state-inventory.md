...
| `cli_output_optional` | Partial: `python_pytest_oracle_shapes`, `python_call_argument_shape`, `python_cli_output_repair_gap`, `first_successful_pr/python-output-gap` | Generic call, log/output, and side-effect shapes can carry missing discriminators such as `log contains "coupon expired"` or `call includes "receipt.sent"` when direct weak evidence exists, with generic placement/verify guidance; simple Click CLI output now keeps known static decorators transparent, routes `click.echo(...)` to `output contains ...`, and pins pytest placement/verify guidance; exact output text equality can close the output/log canonical gap while containment remains conservative weak evidence. | Add argparse output fixtures and richer exit-code-specific verify guidance. |
