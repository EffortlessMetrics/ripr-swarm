...
| `dynamic_unsupported` | Static-limit and generated-file fixture family plus `python-real-repo-evals:dynamic_dispatch_no_packet_eval` | Dynamic dispatch, dynamic import, arbitrary decorator indirection, mocked module, missing import graph, metaprogramming including metaclass declarations, and unsupported syntax limits are visible and fail closed with typed stop reasons; detectable generated Python diffs are excluded before repair routing. The dogfood eval corpus records dynamic dispatch as a related-test/no-packet case with `not_applicable` verify and receipt results. | Keep those limitations and generated-file changes out of first-use and queue projections while repairable Python cards move through agent packets. |
