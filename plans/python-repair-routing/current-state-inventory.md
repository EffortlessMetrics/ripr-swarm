...
| `fastapi_route_optional` | `python_api_route_decorator_repair_gap`, `python-real-repo-evals:decorated_route_status_pytest_receipt` | Simple FastAPI/Flask-shaped route decorators such as `@api.post(...)` can remain static route metadata, changed status-code assignments can produce field/object repair cards, and before/after receipts can close when broad route smoke evidence becomes exact `response.status_code` evidence. | Add response JSON-field repair cards and dynamic client-routing limitations. |
| `cli_output_optional` | Partial: `python_pytest_oracle_shapes`, `python_call_argument_shape`, `first_successful_pr/python-output-gap` | Generic call, log/output, and side-effect shapes can carry missing discriminators such as `log contains "coupon expired"` or `call includes "receipt.sent"` when direct weak evidence exists, with generic placement/verify guidance; exact output text equality can now close the output/log canonical gap while containment remains conservative weak evidence. | Add Click/Typer/argparse output fixtures, output assertion cards, and exit-code-specific verify guidance. |
